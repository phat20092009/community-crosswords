...
class PHP_Crossword 
{
    ...
    var $table = TABLE_CROSSWORDS_KEYWORDS;
	var $groupid = 0;
    var $db;
	 ...
	 * @param int $cols
	 */
    function PHP_Crossword($db=null, $rows = 15, $cols = 15, $catid = 0, $max_words = 15, $max_full_tries = 5)
    {
        $this->rows = (int)$rows;
        $this->cols = (int)$cols;
        $this->groupid = (int)$catid;
	...
	function __getItems()
    {
        ...
        foreach ($this->grid->words as $word){
        	$words[] = $this->db->quote($word->word);
        }
        $words = implode(",", $words);
        $query = "select id, keyword, question from {$this->table} where catid={$this->groupid} and keyword in ({$words})";
        $this->db->setQuery($query);
        $keywords = $this->db->loadAssocList("keyword");
        ...
        for ($i = 0; $i < count($this->grid->words); $i++)
        {
            ...
            $items[] = array(
            	"id"		=> $keywords[$w->word]["id"],
				"word"		=> $w->word,
				"question"	=> $keywords[$w->word]["question"],
				"x"			=> $w->getStartX() + 1,
				"y"			=> $w->getStartY() + 1,
				"axis"		=> $w->axis,
				);
    ...
    {
        $sql = "SELECT question FROM {$this->table} WHERE catid={$this->groupid} AND keyword = '{$word}'";
    ...
    function __loadWords($match, $len_min, $len_max)
    {
        ...
        $sql = "SELECT keyword FROM {$this->table} WHERE
        	published = 1 AND
			catid={$this->groupid} AND
            LENGTH(keyword)<={$len_max} AND
            LENGTH(keyword)>={$len_min} AND
            keyword LIKE '{$match}'
            {$used_words_sql}
            ";
		$this->db->setQuery($sql);
    ...
    function __getUsedWordsSql()
    {
        ...
        for ($i = 0; $i < count($this->grid->words); $i++)
        	$sql .= "AND keyword!='" . $this->grid->words[$i]->word . "' ";
    ...
    {
        $where = "LENGTH(keyword)<={$max_length}";

        $count = $this->__getWordsCount($where);

        if (!$count){
            JFactory::getApplication()->enqueueMessage( JText::_('MSG_INSUFFICIENT_KEYWORDS') );
            return false;
        }

        $n = rand(0, $count-1);

        $sql = "SELECT keyword FROM {$this->table}
            WHERE published=1 AND catid={$this->groupid} AND {$where}
            LIMIT {$n}, 1";
    ...
    function __getWordsCount($where = NULL)
    {
        ...
        $sql = "SELECT COUNT(keyword) FROM {$this->table} 
			WHERE published = 1 AND catid={$this->groupid} {$where_sql}";
    ...
    {
        $sql = "SELECT keyword FROM {$this->table} WHERE 
			published = 1 AND catid = {$this->groupid} AND
			UPPER(keyword) = UPPER('{$word}')";
        $this->db->setQuery($sql);
        $obj = $this->db->loadObject();
        return $obj->keyword ? TRUE : FALSE;
    }
    ...
    function insertWord($word, $question)
    {
        ...
        $sql = "INSERT INTO {$this->table}(catid, keyword, question) 
	...
	function countWordsInGroup($groupid = NULL)
	{
		...
		$sql = "SELECT COUNT(*) FROM {$this->table} WHERE catid={$groupid} and published=1";
		$this->db->setQuery($sql);
		$row = $this->db->loadResult();
		return (int)$row;
	}
	...
	{
		$sql = "SELECT catid FROM {$this->table} where published=1 GROUP BY catid ORDER BY groupid";
		$this->db->setQuery($sql);
    ...
    {
        $sql = "SELECT catid FROM {$this->table} WHERE published=1 AND catid = {$groupid}";
        $this->db->setQuery($sql);
        $row = $this->db->loadResult();
	...
	function removeGroup($groupid = NULL)
	{
		...
		$sql = "DELETE FROM {$this->table} WHERE published=1 AND catid={$groupid}";
		$this->db->setQuery($sql);
		$this->db->query();
