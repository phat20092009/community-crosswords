...
<table class="adminlist">
    <thead>
        <tr>
            ...
            <th width="20"><input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count( $this->crosswords ); ?>);" /></th>
            ...
            <th width="8%" class="title"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_USERNAME' ), 'a.created_by', $this->lists['order_Dir'], $this->lists['order']); ?></th>
            ...
            <th width="4%" class="title"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_QUESTIONS' ), 'a.questions', $this->lists['order_Dir'], $this->lists['order']); ?></th>
            <th width="4%" class="title"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_ROWS' ), 'a.rows', $this->lists['order_Dir'], $this->lists['order']); ?></th>
            <th width="4%" class="title"><?php echo JHTML::_( 'grid.sort', JText::_( 'LBL_COLUMNS' ), 'a.columns', $this->lists['order_Dir'], $this->lists['order']); ?></th>
    ...
    $k = 0;
    $i=0;
    foreach($this->crosswords as $row){
    $checked    = JHTML::_( 'grid.id', $i, $row->id );
    $link = 'index.php?option='.$option.'&task=edit_crossword&cid='.$row->id;
    ?>
    <tr class="<?php echo "row$k"; ?>">
        ...
        <td>
            <a href="<?php echo $link?>" onmouseover=""><?php echo $this->escape($row->title); ?></a>
        </td>
        <td>
        ...
        <td align="center">
            <?php
                ...
                if($row->published == 1){
                    $img = 'published.png';
                    $alt = 'LBL_UNPUBLISH';
                    $publishUrl = JRoute::_( 'index.php?option='.$option.'&task=unpublish_crosswords&cid[]='. $row->id );
                }else{
                    $img = 'unpublished.png';
                    $alt = 'LBL_PUBLISH';
                    $publishUrl = JRoute::_( 'index.php?option='.$option.'&task=publish_crosswords&cid[]='. $row->id );
                }
             ?>
        ...
        <td align="center">
            <?php echo $this->escape($row->questions); ?>
        </td>
        <td align="center">
            <?php echo $this->escape($row->rows); ?>
        </td>
        <td align="center">
        	<?php echo $this->escape($row->columns); ?>
        </td>
        <td>
...
<input type="hidden" name="keyid" value="<?php echo JRequest::getInt('keyid');?>" />
